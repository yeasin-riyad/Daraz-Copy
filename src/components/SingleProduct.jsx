...
const SingleProduct = ({ product }) => {
  ...
  const { data: cartData } = useMyCartItemsQuery(session?.user?.email);
  const [AddToFavorite] = useAddToFavoriteMutation();
  const { data: favoriteItems, refetch } = useMyfavoriteItemsQuery(
    session?.user?.email
  );
  const id = favoriteItems?.find((item) =>
    item.product.id == product.id ? true : false
  );

  const item = cartData?.find(
    (SingleProduct) => SingleProduct.product.id == product.id
  );
  ...
  return (
    <div className="group flex flex-col border-2 border-gray-300 bg-white shadow-md rounded-lg overflow-hidden transform transition-all duration-300 hover:scale-105">
      ...
      <div className={`${id ? "opacity-100":""} absolute inset-0 flex items-start justify-end p-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none`}>
        <div className="flex flex-col space-y-2 text-gray-500 pointer-events-auto">
          <button
            onClick={() => toast.success("item added successfully")}
          ...
          <button
            className={`p-2 bg-gray-200 rounded-full hover:bg-orange-500 hover:text-white 
               ${
                 id ? "bg-orange-500 text-white" : ""
               }`}
            title={id ?"Your Favorite Item":"Add to Favorites"}
      ...
      <div className="flex flex-col justify-between p-4 mt-4 flex-grow">
        <div>
          ...
          <ProductPrice
            price={item ? item?.totalPrice : product?.price}
            discountPe={product.discountPercentage}
          />
